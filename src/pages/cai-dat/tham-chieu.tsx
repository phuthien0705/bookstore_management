    ...
    <>
      ...
      <Card>
          ...
          <CardBody className="flex flex-row gap-10">
            <Input variant="outlined" label="Số lượng nhập tối thiểu" defaultValue={data?.SoLuongNhapToiThieu} onChange={(e) => setEntryMin(Number(e.target.value))}/>
            <Input variant="outlined" label="Số lượng tồn tối thiểu" defaultValue={data?.SoLuongTonToiThieu} onChange={(e) => setLeftMax(Number(e.target.value))}/>
          </CardBody>
        </Card>
        <Card className="mt-10">
          ...
          <CardBody className="flex flex-row gap-10">
            <div className="flex flex-col gap-6" style={{width: "100%"}}>
                <Input variant="outlined" label="Số tiền nợ tối đa" defaultValue={data?.CongNoToiDa} onChange={(e) => setDebtMax(Number(e.target.value))}/>
                <Input variant="outlined" label="Tỷ lệ đơn giá bán" defaultValue={data?.TyLeDonGia.toString()} onChange={(e) => setPriceRatio(Number(e.target.value))}/>
            </div>
            <div className="flex flex-col gap-6" style={{width: "100%"}}>
              <Input variant="outlined" label="Tồn tối thiểu sau bán" defaultValue={data?.TonKhoToiThieuSauBan} onChange={(e) => setLefAfterSellMin(Number(e.target.value))}/>
            </div>
          </CardBody>
        </Card>
        <div className="flex justify-end mt-8">
          ...
          }  defaultChecked={data?.SuDungQuyDinh == true} onChange={(e) => setApply(e.target.checked)}/>
